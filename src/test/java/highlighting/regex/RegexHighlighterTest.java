...
class RegexHighlighterTest {
  ...
  void keepsJavadocInsteadOfBlockComment() {
    ...
    assertEquals(1, regions.size());
    assertEquals(
        new HighlightRegion(0, 14, MiniJavaColours.JAVADOC_COMMENT_COLOUR), regions.get(0));
  }
  ...
  void adjacentRegionsDoNotOverlap() {
    ...
    assertEquals(new HighlightRegion(0, 3, MiniJavaColours.STRING_LITERAL_COLOUR), regions.get(0));
    assertEquals(new HighlightRegion(3, 6, MiniJavaColours.CHAR_LITERAL_COLOUR), regions.get(1));
  }
